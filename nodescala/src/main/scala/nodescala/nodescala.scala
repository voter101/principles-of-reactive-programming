...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    var listener = createListener(relativePath)
    var listenerSubscription = listener.start()
    var subscriptionProcess = Future.run() { sub => 
        async {
            while (sub.nonCancelled) {
                val (request, exchange) = await { listener.nextRequest() }
                respond(exchange, sub, handler(request))
            }
        }
    }

    Subscription(listenerSubscription, subscriptionProcess)
  }
