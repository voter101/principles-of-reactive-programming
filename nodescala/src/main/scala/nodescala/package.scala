...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
        var p = Promise[T]()
        fs.map { x => x onComplete { p.tryComplete(_) } }
        p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = if (f.isCompleted) Await.result(f, 0 nanos) else throw new NoSuchElementException 
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
        ...
        f onComplete { _ => promise tryComplete Try(cont(f)) }
        promise.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
        var promise = Promise[S]()
        f onComplete { t => promise tryComplete Try(cont(t)) }
        promise.future
    }
